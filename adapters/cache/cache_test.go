...
func TestManifestCodecRoundTripsStrictPolicyFormat(t *testing.T) {
	...
	exactEncoded, err := exact.Encode(manifest)
	if err != nil {
		t.Fatalf("Encode(at exact limit) error = %v", err)
	}
	if _, err := exact.Decode(exactEncoded); err != nil {
...
func TestNewManifestCacheUsesIsolatedRevisionKeys(t *testing.T) {
	...
	manifestCache, err := New(Config{Namespace: "service", Backend: backend, Clock: clock, TTL: cache.TTLPolicy{TTL: time.Minute}})
	if err != nil {
...
func cacheManifest(revision authorization.Revision) policy.Manifest {
	return policy.Manifest{Format: policy.FormatV1, Revision: revision, Algorithm: policy.AlgorithmDenyOverrides, Policies: []policy.Record{}}
}
