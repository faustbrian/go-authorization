...
`go-authorization` is an application-oriented authorization engine for typed Go
...
- atomic PostgreSQL manifest persistence with optimistic revision checks and a
  reusable `go-migrations` migration;
...
- fail-closed standard-library HTTP and native `go-jsonrpc` integration;
- bounded `go-log` audit and `go-telemetry` metrics/trace adapters; and
- explicit advisory `go-cache` manifest integration.
