...
The `authcache` package configures a typed `go-cache` cache for versioned policy
...
`authcache.RepositoryLoader` returns a `go-cache` loader that reports a hit only
