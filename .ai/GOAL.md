...
- Consume `go-authentication` principals without making `go-authentication`
  depend on authorization.
- HTTP and JSON-RPC middleware/adapters that map decisions to transport errors.
- `go-log` audit events and `go-telemetry` metrics/traces with bounded labels.
- `go-cache` integration only through explicit adapters.
