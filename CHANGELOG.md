...
- Canonical `authhttp` package, dependency-neutral authenticated-principal
  mapper, and native fail-closed `go-jsonrpc` middleware.
- Failure-isolated decision instrumentation, bounded `go-log` audit events,
  `go-telemetry`-compatible OpenTelemetry metrics and spans, and an explicit
  advisory `go-cache` manifest adapter.
...
- Executable consumer contracts for published `go-authentication`, `go-log`,
  and `go-telemetry` modules, including authentication claim collections.
